...
<p class="action">
	<a href="<?php echo $this->url('purchase', 'index') ?>"<?php if (is_null($category)) {
    echo ' class="current-shop-category"';
} ?>>
		<?php echo htmlspecialchars(Flux::message('AllLabel')) ?> (<?php echo number_format($total) ?>)
	</a>
<?php foreach ($categories as $catID => $catName): ?>
	/
	<a href="<?php echo $this->url('purchase', 'index', ['category' => $catID]) ?>"<?php if (!is_null($category) && $category === (string) $catID) {
    echo ' class="current-shop-category"';
} ?>>
	...
	<tr>
		...
		<td>
			...
			<p class="shop-item-action">
				...
				/ <a href="<?php echo $this->url('itemshop', 'edit', ['id' => $item->shop_item_id]) ?>">Modify</a>
				<?php endif ?>
				<?php if ($auth->allowedToDeleteShopItem): ?>
				/ <a href="<?php echo $this->url('itemshop', 'delete', ['id' => $item->shop_item_id]) ?>"
		...
		<td class="shop-item-cost-qty">
			...
			<p class="shop-item-action">
				...
				<a href="<?php echo $this->url('purchase', 'add', ['id' => $item->shop_item_id]) ?>"><strong>Add to Cart</strong></a>
				<?php endif ?>
			</p>
		</td>
	</tr>
