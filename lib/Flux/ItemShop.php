...
class Flux_ItemShop
{
    ...
    public function add($itemID, $categoryID, $cost, $quantity, $info, $useExisting = 0)
    {
        ...
        $res = $sth->execute([$itemID, $categoryID, $quantity, $cost, $info, $useExisting]);
    ...
    public function getItem($shopItemID)
    {
        ...
        if ($this->server->isRenewal) {
            $fromTables = ["$db.item_db_re", "$db.item_db2_re"];
        } else {
            $fromTables = ["$db.item_db", "$db.item_db2"];
        }
