...
class Flux_LoginServer extends Flux_BaseServer
{
    ...
    public function register($username, $password, $confirmPassword, $email, $email2, $gender, $birthdate, $securityCode)
    {
        ...
        } elseif (!in_array(strtoupper($gender), ['M', 'F'])) {
        ...
        $sth = $this->connection->getStatement($sql);
        $res = $sth->execute([$username, $password, $email, $gender, (int) $this->config->getGroupID(), date('Y-m-d', $birthdatestamp)]);
        ...
        if ($res) {
            ...
            $sth->execute([$idres->account_id, $username, $password, $gender, $email, $_SERVER['REMOTE_ADDR']]);
    ...
    public function permanentlyBan($bannedBy, $banReason, $accountID)
    {
        ...
        if ($sth->execute([$accountID, $bannedBy, $banReason])) {
    ...
    public function unban($unbannedBy, $unbanReason, $accountID)
    {
        ...
        if ($sth->execute([$accountID, $unbannedBy, $unbanReason])) {
    ...
    public function addIpBan($bannedBy, $banReason, $unbanTime, $ipAddress)
    {
        ...
        if ($sth->execute([$ipAddress, $bannedBy, $unbanTime, $banReason])) {
            $sql = "INSERT INTO {$this->loginDatabase}.ipbanlist (list, reason, rtime, btime) ";
            $sql .= 'VALUES (?, ?, ?, NOW())';
            $sth = $this->connection->getStatement($sql);

            return $sth->execute([$ipAddress, $banReason, $unbanTime]);
        } else {
            return false;
        }
    }
    ...
    public function removeIpBan($unbannedBy, $unbanReason, $ipAddress)
    {
        ...
        if ($sth->execute([$ipAddress, $unbannedBy, $unbanReason])) {
            $sql = "DELETE FROM {$this->loginDatabase}.ipbanlist WHERE list = ?";
            $sth = $this->connection->getStatement($sql);

            return $sth->execute([$ipAddress]);
        } else {
            return false;
        }
    }
    ...
    public function depositCredits($targetAccountID, $credits, $donationAmount = null)
    {
        ...
        if (!$sth->execute([$targetAccountID]) || !$sth->fetch()->accountExists) {
    ...
    public function getPrefs($accountID, array $prefs = [])
    {
        $sql = "SELECT account_id FROM {$this->loginDatabase}.`login` WHERE account_id = ? LIMIT 1";
        $sth = $this->connection->getStatement($sql);

        if ($sth->execute([$accountID]) && ($char = $sth->fetch())) {
            ...
            if ($sth->execute($bind)) {
                ...
                foreach ($prefsArray as $pref => $value) {
                    if (array_key_exists($pref, $update)) {
                        $id = $update[$pref];
                        $usth->execute([$value, $id]);
                    } else {
                        $isth->execute([$accountID, $pref, $value]);
                    }
                }
    ...
    {
        $prefs = $this->getPrefs($accountID, [$pref]);
    ...
    {
        return $this->setPrefs($accountID, [$pref => $value]);
    }
