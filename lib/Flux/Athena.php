...
class Flux_Athena
{
    ...
    public function __construct(Flux_Config $charMapConfig, Flux_LoginServer $loginServer, Flux_CharServer $charServer, Flux_MapServer $mapServer)
    {
        ...
        if (!$resetDenyMaps) {
            $this->resetDenyMaps = ['sec_pri'];
        } elseif (!is_array($resetDenyMaps)) {
            $this->resetDenyMaps = [$resetDenyMaps];
        } else {
    ...
    public function transferCredits($fromAccountID, $targetCharName, $credits)
    {
        ...
        if (!$sth->execute([$targetCharName]) || !($char = $sth->fetch())) {
            // Unknown character.
            return -3;
        }
        ...
        if (!$sth->execute([$fromAccountID, $targetAccountID]) || $sth->fetch()->accounts != 2) {
        ...
        if ($this->loginServer->depositCredits($fromAccountID, -$credits)) {
            ...
            if (!$this->loginServer->depositCredits($targetAccountID, $credits)) {
                ...
                // Log transfer.
                $sth->execute([$fromAccountID, $targetAccountID, $targetCharID, $credits]);
    ...
    public function charExists($charID)
    {
        ...
        if ($sth->execute([$charID]) && ($char = $sth->fetch()) && $char->char_id) {
            return true;
        } else {
            return false;
        }
    ...
    public function charBelongsToAccount($charID, $accountID)
    {
        ...
        if ($sth->execute([$charID, $accountID]) && ($char = $sth->fetch()) && $char->char_id) {
            return true;
        } else {
            return false;
        }
    ...
    public function accountHasOnlineChars($accountID)
    {
        ...
        if ($sth->execute([$accountID]) && ($char = $sth->fetch()) && $char->char_id) {
            return true;
        } else {
            return false;
        }
    ...
    public function getCharacter($charID)
    {
        ...
        if ($sth->execute([$charID]) && ($char = $sth->fetch())) {
            return $char;
        } else {
            return false;
        }
     ...
     */
    public function getPrefs($charID, array $prefs = [])
    {
            ...
            if ($sth->execute($bind)) {
                ...
                foreach ($prefsArray as $pref => $value) {
                    if (array_key_exists($pref, $update)) {
                        ...
                        $isth->execute([$char->account_id, $charID, $pref, $value]);
                    }
                }
    ...
    {
        $prefs = $this->getPrefs($charID, [$pref]);
    ...
    {
        return $this->setPrefs($charID, [$pref => $value]);
    }
