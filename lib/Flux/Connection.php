...
class Flux_Connection
{
    ...
    private function connect(Flux_Config $dbConfig)
    {
        ...
        $persistent = [PDO::ATTR_PERSISTENT => (bool) $dbConfig->getPersistent()];
     ...
     * @return PDOStatement
     */
    public function getStatement($statement, $options = [])
    {
        $dbh = $this->getConnection();
        $sth = $dbh->prepare($statement, $options);
        @$sth->setFetchMode(PDO::FETCH_CLASS, 'Flux_DataObject', [null, ['dbconfig' => $this->dbConfig]]);
     ...
     * @return PDOStatement
     */
    public function getStatementForLogs($statement, $options = [])
    {
        $dbh = $this->getLogsConnection();
        $sth = $dbh->prepare($statement, $options);
        @$sth->setFetchMode(PDO::FETCH_CLASS, 'Flux_DataObject', [null, ['dbconfig' => $this->logsDbConfig]]);
    ...
    public function isCaseSensitive($database, $table, $column, $useLogsConnection = false)
    {
        ...
        $sth = $this->$stm($sql);
        $sth->execute([$database, $table, $column]);
