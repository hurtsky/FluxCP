...
class Flux_Template
{
    ...
    public function __construct(Flux_Config $config)
    {
        ...
        if (file_exists($this->themePath.'/'.$this->themeName.'/manifest.php')) {
            ...
            if (!empty($manifest['inherit'])) {
                ...
                $this->parentTemplate = new self($config);
            }
        }
    }
     ...
     */
    public function render(array $dataArr = [])
    {
        // GZip compression.
        ...
        if (Flux::config('OutputCleanHTML') && !$tidyIgnore && function_exists('tidy_repair_string')) {
            ...
            $content = tidy_repair_string($content, ['indent' => true, 'wrap' => false, 'output-xhtml' => true], 'utf8');
            echo $content;
        }
    }
    ...
    public function getSubMenuItems($moduleName = null)
    {
        ...
        foreach ($menus->toArray() as $actionName => $menuName) {
            if ($auth->actionAllowed($moduleName, $actionName)) {
                $allowedItems[] = ['name' => $menuName, 'module' => $moduleName, 'action' => $actionName];
            }
        }
     ...
     */
    public function url($moduleName, $actionName = null, $params = [])
    {
        $defaultAction = Flux_Dispatcher::getInstance()->defaultAction;
        $serverProtocol = '';
        $serverAddress = '';
     ...
     */
    public function getPaginator($total, array $options = [])
    {
        $paginator = new Flux_Paginator($total, $this->url($this->moduleName, $this->actionName, ['_host' => false]), $options);
    ...
    public function emblem($guildID, $serverName = null, $athenaServerName = null)
    {
        ...
        return $this->url('guild', 'emblem',
            ['login' => $serverName, 'charmap' => $athenaServerName, 'id' => $guildID]);
    }
