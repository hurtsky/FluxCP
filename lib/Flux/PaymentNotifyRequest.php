...
class Flux_PaymentNotifyRequest
{
    ...
    public function process()
    {
        ...
        if (in_array($received_from, $allowed_hosts) && $this->verify()) {
            ...
            $accountEmails = array_merge([$this->myBusinessEmail], $accountEmails->toArray());
            ...
            if (!in_array($receiverEmail, $accountEmails)) {
                ...
                $customArray = $customArray && is_array($customArray) ? $customArray : [];
                        ...
                        if (!$res) {
                            ...
                            if ($amount >= $minimum) {
                                ...
                                if ($holdHours) {
                                    ...
                                    $sth = $servGroup->connection->getStatement($sql);
                                    $sth->execute([$accountID, $payerEmail]);
                                    $res = $sth->fetch();
            ...
            if (!in_array($received_from, $allowed_hosts) && Flux::config('PaypalHackNotify')) {
                ...
                foreach ($accountEmails as $email) {
                    $sent = $mail->send($email, '['.Flux::config('SiteTitle').'] Paypal hack', $tmpl, ['_ignoreTemplate' => true]);
                }
    ...
    private function verify()
    {
        ...
        if (!$fp) {
            ...
            $this->logPayPal('Sent %d bytes of transaction data. Request size: %d bytes.', strlen($qString), fwrite($fp, $request));
