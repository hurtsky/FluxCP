...
class Flux
{
     ...
     * @var array
     */
    public static $loginAthenaGroupRegistry = [];
     ...
     * @var array
     */
    public static $athenaServerRegistry = [];
    ...
    {
        $required = ['appConfigFile', 'serversConfigFile'];
     ...
     * @param arary  $options
     */
    public static function config($key, $value = null, $options = [])
    {
        if (!is_null($value)) {
     ...
     * @param arary  $options
     */
    public static function message($key, $value = null, $options = [])
    {
        if (!is_null($value)) {
    ...
    {
        $basename = basename(str_replace(' ', '', ucwords(str_replace(['/', '\\', '_'], ' ', $filename))), '.php').'.cache.php';
    ...
    public static function parseServersConfigFile($filename)
    {
        ...
        $options = ['overwrite' => false, 'force' => true]; // Config::set() options.
        $serverNames = [];
        $athenaServerNames = [];
        ...
        foreach ($config->getChildrenConfigs() as $topConfig) {
            ...
            $athenaServerNames[$serverName] = [];

            $topConfig->setDbConfig([], $options);
            $topConfig->setLogsDbConfig([], $options);
            $topConfig->setLoginServer([], $options);
            $topConfig->setCharMapServers([], $options);

            $dbConfig = $topConfig->getDbConfig();
            $logsDbConfig = $topConfig->getLogsDbConfig();
            $loginServer = $topConfig->getLoginServer();

            foreach ([$dbConfig, $logsDbConfig] as $_dbConfig) {
            ...
            foreach ($topConfig->getCharMapServers()->getChildrenConfigs() as $charMapServer) {
                ...
                $charMapServer->setCharServer([], $options);
                $charMapServer->setMapServer([], $options);
    ...
    private static function registerAthenaServer($serverName, $athenaServerName, Flux_Athena $athenaServer)
    {
        if (!array_key_exists($serverName, self::$athenaServerRegistry) || !is_array(self::$athenaServerRegistry[$serverName])) {
            self::$athenaServerRegistry[$serverName] = [];
        }
        ...
        foreach ($loginAthenaGroups as $loginAthenaGroup) {
            ...
            if ($sth->execute() && ($txn = $sth->fetchAll())) {
                foreach ($txn as $t) {
                    ...
                    if ($sth->execute([$t->txn_id]) && ($r = $sth->fetch()) && $r->id) {
                        $cancel[] = $t->txn_id;
            ...
            foreach ($accept as $txn) {
                ...
                $sth3->execute([$txn->account_id, $txn->payer_email]);
                $row = $sth3->fetch();

                if (!$row) {
                    $sth2->execute([$txn->account_id, $txn->payer_email]);
                }
    ...
    {
        $tbl = self::config('FluxTables.AccountCreateTable');
    ...
    {
        $equiplocations = self::config('EquipLocations')->toArray();
    ...
    {
        $equipupper = self::config('EquipUpper')->toArray();
    ...
    {
        $equipjobs = self::config('EquipJobs')->toArray();
    ...
    {
        $neutral = self::config('Elements.0');
        $element = self::config("Elements.$ele");
    ...
    {
        $race = self::config("MonsterRaces.$race");
    ...
    {
        $size = self::config("MonsterSizes.$size");
