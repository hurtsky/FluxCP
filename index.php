...
if (ini_get('magic_quotes_gpc')) {
    $gpc = [&$_GET, &$_POST, &$_REQUEST, &$_COOKIE];
    foreach ($gpc as &$arr) {
...
try {
    ...
    $dispatcher->dispatch([
        ...
        'missingActionModuleAction'	=> Flux::config('DebugMode') ? ['errors', 'missing_action'] : ['main', 'page_not_found'],
        'missingViewModuleAction'	  => Flux::config('DebugMode') ? ['errors', 'missing_view'] : ['main', 'page_not_found'],
    ]);
} catch (Exception $e) {
